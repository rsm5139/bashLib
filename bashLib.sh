...
#                                  after the options, or will
#                                  replace <<file>> within the
#                                  options.
...
function commandAll() 
{
  function usage()
  {
    ...
    echo "  --options \"<options/flags>\": String of options and flags to be passed to the command. The filename will be placed after the options, or will replace <<file>> within the options."
    echo "        EXP: --options \"-rf\" command"
    echo "        RESULT: command -rf filename"
    echo "        EXP: --options \"--file \\\"<<file>>\\\"\" command"
    echo "        RESULT: command --file \"filename\""
  ...
  # Set defaults
  command_str=""   # we'll just echo the file names by default
  prefix=""
  options=""
  ext=""
  r=1
